/* ============================================================================
   PPX AI Service – v2.11.0 (Production-ready hybrid)
   Additions (kompatibel, minimal-invasiv):
   • Identity-Intent (offline) inkl. dynamischer Antwort aus cfg.brand/tagline
   • Alias/Synonym-Layer (offline) → robustere Erkennung („Laden“, „Karte“…)
   • Empathie-Trigger erweitert (einsam/keine Freunde/wütend/sauer…)
   • Very-Short-Bypass für W-Fragen (wer/was/who/what)
   • freeLineBeforeFlow: Kurzantwort vom Worker + danach Flow (optional)
   • Debug-Events (lokal) für schnelle Abnahme-Tests
   • Kontaktformular-Prio bleibt: contactform → kontakt → Event
============================================================================ */
(function () {
  'use strict';
  ...
  function showNote(txt){ appendToView(el('div',{class:'ppx-note',style:{background:'rgba(255,255,255,.10)',border:'1px solid rgba(255,255,255,.28)',borderLeft:'4px solid var(--ppx-accent,#c9a667)',borderRadius:'12px',padding:'8px 10px',marginTop:'8px'}},txt)); }
  ...
  // ---------- tool alias & openFlow -----------------------------------------
  function toolAlias(n){ n=String(n||'').toLowerCase(); if(n==='öffnungszeiten'||n==='oeffnungszeiten'||n==='hours') return 'hours';
...
#ppx-panel .ppx-ai-dock.busy .ai-send{opacity:.65;pointer-events:none}");
    ...
    try{ if(panelFooter){ panel.insertBefore($dock, panelFooter); } else if(v && v.parentNode===panel && v.nextSibling){ panel.insertBefore($dock, v.nextSibling); } else panel.appendChild($dock); }catch(e){ panel.appendChild($dock); }
  ...
  // ---------- doWorker (with freeLineBeforeFlow) -----------------------------
  async function doWorker(q, systemPromptOverride){
    var A=aiCfg()||{}, freeBefore=!!(A.settings&&A.settings.freeLineBeforeFlow);
    var bWrap=appendToView(bubble('bot','⏳ …')), bBot=bWrap && bWrap.querySelector('.ppx-ai-bubble'), res=null;
    ...
    var allow=(A.allowlist||['reservieren','kontakt','öffnungszeiten','speisen','faq']).map(function(s){return String(s).toLowerCase();});
    var tool=(res.tool||'').toLowerCase();

    // Free line before flow (if desired)
    if(res.text && (!tool || allow.indexOf(tool)!==-1) && bBot){ bBot.innerHTML=linkify(esc(res.text)); } else if(bWrap && bWrap.parentNode){ try{ bWrap.parentNode.removeChild(bWrap); }catch(_e){} }

    if(!tool || allow.indexOf(tool)===-1 || tool==='kontakt'){
      // No routing (keep short answer or go to contact offer)
    ...
    if(freeBefore && res.text){ /* already shown */ }
    if(st().activeFlowId && st().activeFlowId!==tool){ /* pause hook */ }
  ...
  async function send(){
    ...
    // Smalltalk/Identity first
    var sm=detectSmalltalk(q); if(sm){ appendToView(bubble('bot', esc(sm))); resetUnknown(); resetOos(); moveThreadToEnd(); dbgPush({type:'smalltalk',q:q}); return; }
